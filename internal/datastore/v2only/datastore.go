...
//
// When sourceIDs is non-empty, the hourly counts are scoped to detections from those audio
// sources (feature: per-source analytics filter).
func (ds *Datastore) GetBatchHourlyOccurrences(ctx context.Context, startDate, endDate string, species []string, minConfidence float64, sourceIDs ...uint) (map[string][24]int, error) {
	if len(species) == 0 {
	...
	for _, segment := range ds.splitByZoneOffset(startOfDay, endOfDay) {
		part, err := ds.detection.GetBatchHourlyOccurrences(ctx, flatLabelIDs, segment.start, segment.end, segment.offset, minConfidence, sourceIDs...)
		if err != nil {
			return nil, errors.New(err).
				Component("datastore").
				Category(errors.CategoryDatabase).
...
func (ds *Datastore) GetHourlyAnalyticsData(ctx context.Context, date, species string, sourceIDs ...uint) ([]datastore.HourlyAnalyticsData, error) {
	...
	v2Data, err := ds.detection.GetHourlyDistribution(ctx, start, end, ds.zoneOffsetSeconds(start), labelIDs, nil, sourceIDs...)
	if err != nil {
		return nil, err
	}
...
func (ds *Datastore) GetDailyAnalyticsData(ctx context.Context, startDate, endDate, species string, sourceIDs ...uint) ([]datastore.DailyAnalyticsData, error) {
	...
	v2Data, err := ds.detection.GetDailyAnalytics(ctx, start, end, ds.zoneOffsetSeconds(dateRangeOffsetAnchor(start, end)), labelIDs, nil, sourceIDs...)
	if err != nil {
		return nil, err
	}

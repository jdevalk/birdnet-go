...
func (r *detectionRepository) buildAnalyticsBaseQuery(ctx context.Context, start, end int64, labelIDs []uint, modelID *uint, sourceIDs ...uint) *gorm.DB {
	detTable := r.tableName()
	revTable := r.reviewsTable()
	query := r.db.WithContext(ctx).Table(fmt.Sprintf("%s d", detTable)).
		Joins(fmt.Sprintf("LEFT JOIN %s dr ON d.id = dr.detection_id", revTable)).
...
func (r *detectionRepository) GetHourlyDistribution(ctx context.Context, start, end int64, tzOffsetSeconds int, labelIDs []uint, modelID *uint, sourceIDs ...uint) ([]HourlyDistributionData, error) {
	...
	hourExpr := r.hourFromUnixExpr("d.detected_at", tzOffsetSeconds)
	query := r.buildAnalyticsBaseQuery(ctx, start, end, labelIDs, modelID, sourceIDs...).
...
func (r *detectionRepository) GetDailyAnalytics(ctx context.Context, start, end int64, tzOffsetSeconds int, labelIDs []uint, modelID *uint, sourceIDs ...uint) ([]DailyAnalyticsData, error) {
...
func (r *detectionRepository) GetNewSpecies(ctx context.Context, start, end int64, limit, offset int, sourceIDs ...uint) ([]NewSpeciesData, error) {
	...
	// per-source when filtering, in the outer query so detection_id/confidence come from the
	// same source set, and in the count_in_period subquery so the reported count matches the rest
	// of the row. Empty when sourceIDs is empty, collapsing this to the unscoped query.
	_, sourceClauseCount, sourceArgs := buildSourceFilterClauses(sourceIDs, "", "d3")
	_, sourceClauseInner, _ := buildSourceFilterClauses(sourceIDs, "", "d2")
	...
	rawSQL := fmt.Sprintf(`
		SELECT
			...
			(
				...
				WHERE l3.scientific_name = species_first.scientific_name
					...
					AND (dr3.verified IS NULL OR dr3.verified != ?)%s
			) as count_in_period
		FROM (
			SELECT
				l2.scientific_name,
		...
		LIMIT ? OFFSET ?
	`,
		r.tableName(), r.labelsTable(), r.reviewsTable(), sourceClauseCount, // count_in_period subquery
		r.tableName(), r.labelsTable(), r.reviewsTable(), sourceClauseInner, // species_first derived table
		r.labelsTable(), r.tableName(), r.reviewsTable(), sourceClauseOuter, // outer representative-detection join
	)

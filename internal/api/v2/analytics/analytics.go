...
func (c *Handler) fetchSpeciesSummaryData(ctx echo.Context, startDate, endDate string, sourceIDs []uint) ([]datastore.SpeciesSummaryData, time.Duration, error) {
	dbStart := time.Now()
	queryCtx, cancel := withAnalyticsTimeout(ctx)
	defer cancel()
...
func (c *Handler) GetTimeOfDayDistribution(ctx echo.Context) error {
	...
	queryCtx, cancel := withAnalyticsTimeout(ctx)
	defer cancel()
	hourlyData, err := c.DS.GetHourlyDistribution(queryCtx, startDate, endDate, querySpecies, sourceIDs...)
	if err != nil {
...
func (c *Handler) GetBatchHourlySpeciesData(ctx echo.Context) error {
	...
	// Process all species
	results, processingErrors := c.processHourlyBatchSpecies(ctx, speciesParams, startDate, endDate, sourceIDs, ip, path, minConfidence)
...
//
// sourceIDs (when non-empty) restricts the batch query to the listed audio sources.
func (c *Handler) processHourlyBatchSpecies(ctx echo.Context, speciesParams []string, startDate, endDate string, sourceIDs []uint, ip, path string, minConfidence float64) (results map[string][]HourlyDistribution, processingErrors []string) {
	...
	queryCtx, cancel := withAnalyticsTimeout(ctx)
	defer cancel()
	hourlyByName, err := c.DS.GetBatchHourlyOccurrences(queryCtx, startDate, endDate, queryNames, minConfidence, sourceIDs...)
	if err != nil {
...
// sourceIDs (when non-empty) restricts every per-species query to the listed audio sources.
func (c *Handler) processDailyBatchSpecies(ctx echo.Context, uniqueSpecies []string, startDate, endDate string, sourceIDs []uint, ip, path string) (results map[string]SpeciesDailyData, processingErrors []string) {
